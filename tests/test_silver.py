...
def test_clean_bronze_drops_null_customer_id(spark):
    events = [
        {
            "order_id": "o1",
            "customer_id": None,
...
def test_clean_bronze_dedup_tiebreaks_on_ingested_at(spark):
    events = [
        {
            "order_id": "o1",
            "customer_id": "c1",
        ...
        },
        {
            ...
            "quantity": 1,
            "status": "delivered",
            "event_time": "2026-01-01T00:00:00+00:00",  # same event_time
            "ingested_at": "2026-01-01T00:05:00+00:00",  # later replay wins
        },
    ]
    bronze_df = events_to_bronze_df(spark, events)
    silver_df = clean_bronze(bronze_df)
